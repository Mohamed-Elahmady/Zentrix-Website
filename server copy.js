...
// Settings micro-cache — avoids a synchronous fs.readFileSync on EVERY request
// (which stacks up when dashboard, settings, and multiple middleware all call this).
// 30s TTL is fine: settings only change when admin explicitly saves them, and
// saveSettings() invalidates the cache immediately.
...
function saveSettings(data) {
  settingsCache = null;   // invalidate immediately so the next read is fresh
  settingsCacheTime = 0;
  try {
...
// On Vercel, a serverless function can be FROZEN the instant the HTTP response
// is sent — any promise still in flight at that moment gets silently killed.
// That's exactly what caused lost orders the last time fire-and-forget was tried.
// @vercel/functions' waitUntil() tells the platform to keep the invocation alive
// until the promise settles, even though the response already went out — so we
// get both: an instant reply to the user AND a guaranteed-to-finish Sheets write.
// Outside Vercel (localhost, Railway, Render…) the process simply keeps running,
// so this degrades gracefully to a normal background promise everywhere else.
let _waitUntil = null;
try {
  _waitUntil = require('@vercel/functions').waitUntil;
} catch {
  _waitUntil = null; // package not installed / not running on Vercel — fine, no-op
}
...
// Bounded with a timeout per attempt so a hung Apps Script call can't stall
...
async function sendToGoogleSheets(orderData) {
  ...
  for (let attempt = 1; attempt <= 2; attempt++) {
    try {
      const res = await fetchWithTimeout(sheetsUrl, 8000, {
        method: 'POST',
...
// Block direct access to stray sensitive files in the public folder
// (e.g. the احتياطي.zip backup that can expose source code).
app.use((req, res, next) => {
  if (/\.(zip|bak|sql|env|log)$/i.test(req.path)) {
    return res.status(404).end();
  }
  next();
});

// Serve public static files with caching:
//   - HTML  → no-cache (always revalidate, instant picks up deploys)
//   - JS/CSS → 1 hour  (short enough that new deploys show quickly;
//                        existing cache-buster ?v=N params force new download anyway)
//   - Images → 7 days  (logos/icons rarely change; user-visible favicon hits count)
...
app.post('/api/order', async (req, res) => {
  ...
  // ── Respond to the customer immediately. Everything above this point is pure,
  // in-memory computation with zero I/O, so this comes back in a few milliseconds
  // instead of waiting on a Google Sheets round-trip. The actual persistence
  // (local cache + Sheets) happens right after, kept alive via runInBackground
  // so it still reliably finishes even on serverless — this is what makes it
  // safe to do (unlike the old plain fire-and-forget that lost orders). ──
  res.json({
  ...
  runInBackground((async () => {
    ...
    cachedDashboardData = null; // Invalidate cache so the new order shows up on next dashboard load
  })());
});
...
app.put('/api/admin/orders/:order_id', requireAuth, async (req, res) => {
  ...
  const { shipping_paid, product_paid, status } = req.body;
  const settings = getSettings();
  const sheetsUrl = settings.google_sheets_url;
  ...
  if (fs.existsSync(ordersPath)) {
    try {
      ...
      if (idx !== -1) {
        ...
        if (status && VALID_STATUSES.includes(status)) orders[idx].status = status;
  ...
  if (!orderUpdated) {
    orderUpdated = { order_id, shipping_paid, product_paid, status };
  }
  ...
  if (sheetsUrl) {
    runInBackground(
      fetchWithTimeout(sheetsUrl, 8000, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ action: 'update', order_id, shipping_paid, product_paid, status })
      }).then(r => {
  ...
  if (sheetsUrl) {
    runInBackground(
      fetchWithTimeout(sheetsUrl, 8000, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ action: 'delete_order', order_id })
      }).then(response => {
        if (response.ok) cachedDashboardData = null;
        else console.error('Error deleting order from Google Sheets: status', response.status);
      }).catch(err => console.error('Error deleting order from Google Sheets:', err.message))
    );
  }
});
  ...
  if (sheetsUrl) {
    runInBackground(
      fetchWithTimeout(sheetsUrl, 8000, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ action: 'add_expense', expense: newExpense })
      }).then(response => {
        if (response.ok) cachedDashboardData = null;
        else console.error('Error adding expense to Google Sheets: status', response.status);
      }).catch(err => console.error('Error adding expense to Google Sheets:', err.message))
    );
  }
});
  ...
  if (sheetsUrl) {
    runInBackground(
      fetchWithTimeout(sheetsUrl, 8000, {
        ...
        if (response.ok) cachedDashboardData = null;
        else console.error('Error deleting expense from Google Sheets: status', response.status);
      }).catch(err => console.error('Error deleting expense from Google Sheets:', err.message))
    );
  }
});
  ...
  if (sheetsUrl) {
    runInBackground(
      fetchWithTimeout(sheetsUrl, 8000, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ action: 'add_income', income: newIncome })
      }).then(response => {
        if (response.ok) cachedDashboardData = null;
        else console.error('Error adding income to Google Sheets: status', response.status);
      }).catch(err => console.error('Error adding income to Google Sheets:', err.message))
    );
  }
});
  ...
  if (sheetsUrl) {
    runInBackground(
      fetchWithTimeout(sheetsUrl, 8000, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ action: 'delete_income', id })
      }).then(response => {
        if (response.ok) cachedDashboardData = null;
        else console.error('Error deleting income from Google Sheets: status', response.status);
      }).catch(err => console.error('Error deleting income from Google Sheets:', err.message))
    );
  }
});
